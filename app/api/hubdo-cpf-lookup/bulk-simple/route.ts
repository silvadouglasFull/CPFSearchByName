/**
 * API Route: POST /api/hubdo-cpf-lookup/bulk-simple
 * Performs simple bulk HubDo CPF lookup without target name matching
 * Results are fetched and discarded (consumed directly)
 */
...
export async function POST(request: Request): Promise<NextResponse> {
    try {
        ...
        const payload = (await request.json()) as SimpleBulkLookupBody;
        ...
        const created = await service.enqueueBulkJobSimple({
            cpfs: normalizedCpfs,
            mode,
        });
        ...
        return NextResponse.json({ error: `Simple bulk HubDo lookup failed: ${message}` }, { status: 500 });
    }
}
