/**
 * API Route: POST /api/hubdo-cpf-lookup/bulk-find-match
 * Performs bulk HubDo CPF lookup that stops when target name is found
 */
...
export async function POST(request: Request): Promise<NextResponse> {
    try {
        ...
        const payload = (await request.json()) as FindMatchBulkLookupBody;
        const mode = payload.mode ?? 'normal';
        const targetName = payload.targetName?.trim() ?? '';

        if (!targetName) {
            return NextResponse.json(
                { error: 'Target name is required for find-match mode.' },
                { status: 400 },
            );
        }
        ...
        const created = await service.enqueueBulkJobFindMatch({
            cpfs: normalizedCpfs,
            mode,
            targetName,
        });
        ...
        return NextResponse.json({ error: `Find-match bulk HubDo lookup failed: ${message}` }, { status: 500 });
    }
}
